      ...
      <div class="block-content block-content-full">
        <div class="row">
          <div class="col-lg-12 space-y-5">
            ...
            <form id="edit_user" class="space-y-4" action="{{ route('admin.user.update', ['user' => $user]) }}" method="POST" enctype="multipart/form-data">
              @csrf
              @method('PUT')
              ...
              <div class="row">
                <div class="col-9">
                  <div class="row mb-3">
                    ...
                    <div class="col-sm-9">
                      <input type="text" id="email" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ $user->email }}"
                  ...
                  <div class="row mb-3">
                    ...
                    <div class="col-sm-9">
                      <input type="text" class="form-control @error('fullname') is-invalid @enderror" name="fullname" value="{{ old('fullname', $user->fullname) }}"
                  ...
                  <div class="row mb-3">
                    ...
                    <div class="col-sm-9">
                      <textarea class="form-control @error('bio') is-invalid @enderror" name="bio" rows="3">{{ old('bio', $user->bio) }}</textarea>
                      @error('bio')
                        <div class="invalid-feedback">{{ $message }}</div>
                      @enderror
                    </div>
                    ...
                    <div class="col-sm-9">
                      <select class="form-select @error('group_id') is-invalid @enderror" name="group_id">
                        @foreach($groupUsers as $group)
                          <option value="{{ $group->id }}" @selected(old('group_id', $user->group_id) == $group->id)>{{ $group->name }}</option>
                        @endforeach
                      </select>
                      @error('group_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-3">
                  <div class="avatar-upload">
                    ...
                    <div class="avatar-preview">
                      <div id="imagePreview"
                        style="background-image: url({{ $user->profile_image ?? '/images/default_avatar.png' }});">
                      </div>
                    </div>
                  </div>
